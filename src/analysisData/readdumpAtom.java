	...
	public dumpOneStep readnextNstep(int n) throws IOException {
		...
		if(line.matches("ITEM: ATOMS.*")) {
			for(int i =1;i<=numAtom;i++) {
				...
				atm.setType(Integer.valueOf(fields[1]));
				atm.setXYZ(Double.valueOf(fields[2])+Integer.valueOf(fields[5])*(oneStep.xhi-oneStep.xlo), Double.valueOf(fields[3])+Integer.valueOf(fields[6])*(oneStep.yhi-oneStep.ylo), Double.valueOf(fields[4])+Integer.valueOf(fields[7])*(oneStep.zhi-oneStep.zlo));				

			//	atm.setXYZ(Double.valueOf(fields[2]), Double.valueOf(fields[3]), Double.valueOf(fields[4]));				
				atmlist.add(atm);
			}
		}
